...
export async function POST(request: Request) {
  ...
  const orderId = safeName(String(formData.get("orderId") ?? "order"));
  const files = formData.getAll("files").filter((value): value is File => value instanceof File);

  if (files.length === 0) {
    return NextResponse.json({ error: "No delivery bill images provided." }, { status: 400 });
  }
  ...
  for (const file of files) {
    ...
    const baseName = safeName(file.name.replace(new RegExp(`\\.${extension}$`, "i"), "")) || "delivery-bill";
    const path = `shipping-documents/${orderId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${baseName}.${extension}`;
